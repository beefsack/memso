...
/// Runs as a background task; yields between batches to avoid starving the MCP handler.
async fn reembed_stale(conn: Arc<libsql::Connection>, embedder: Arc<Mutex<Embedder>>) {
    ...
    loop {
        ...
        // Yield to the tokio scheduler between batches so MCP tool calls are not starved.
...
pub async fn run(config: Config) -> Result<()> {
    ...
    // Runs in the background; inject (BM25-only) and search (filtered by model)
    // both degrade gracefully while migration is in progress.
